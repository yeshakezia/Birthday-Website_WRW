...
// ================= Birthday song button (cake stage only) =================
...
if (songBtn) {
  ...
  function playBirthdaySong() {
    ...
    birthdaySong.forEach(([freq, beats]) => {
  ...
  songBtn.addEventListener("click", () => {
    ...
    spawnConfettiBurst(rect.left + rect.width / 2, rect.top, 40);
  });

  // Hide the button once the cake is clicked (gift stage onward)
  cakeEl.addEventListener("click", () => {
    setTimeout(() => { songBtn.hidden = true; }, 1100);
  });

  // Show it again when the animation is repeated
  document.getElementById("btnRepeat").addEventListener("click", () => {
    songBtn.hidden = false;
  });

  // Coming back via #wish deep-link skips the cake stage, so no button
  if (window.location.hash === "#wish") songBtn.hidden = true;
}
